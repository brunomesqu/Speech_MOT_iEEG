...
%% written by Jason Rajsic, March 2011; Harrison Ritz, March 2015 (harrison.ritz@gmail.com)
%% Program requires participants to track 2:6 objects while listening to speech
...
%% set condition: cb(:,2) = attend (0 = speech, 1 = mot); cb(:,3) = speech (0 = clear, 1 = nv12, 2 = nv6)
cb = [[0,1,0,1,0,1]',[0,0,1,1,2,2]'];
switch cb_ans
    case 1
        cb = [[1,2,3,4,5,6]',cb];
    case 2
...
%get datasource
[cond_list] = xlsread('conditions_file/targ_datasource2','cond','A2:C25'); %get CB info
[num, txt] = xlsread('conditions_file/targ_datasource2','set','A2:G217'); %get stim info
...
for block=1:nBlocks
    ...
    while ~keyCode(kbtrig)
        [~,buttonTime,keyCode] = KbCheck(-1); % check for a keypresst
    end
    ...
    for trial=1:nTrials
        ...
        speech = cb(cb(:,1)==cond,3);       %set speech (0= clear, 1 = nv12, 2 = nv6)
        ...
        switch speech
            ...
            case 2
                load(fullfile('stimuli/nv6_stim',sprintf('nv6_%s',txt{rand_sent(sentcount(cond),cond),2})))
        ...
        motfile = sprintf('smsc_dt18_mot_%.3d',stim(count));
